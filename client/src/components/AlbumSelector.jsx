...
export default function AlbumSelector({ token, sdk }) {
	...
	return (
		<div className="flex flex-col my-auto items-center justify-center gap-4 sm:gap-6 md:gap-8 p-4 sm:p-8 md:p-12 bg-gradient-to-b from-[#c4c2bf] to-[#a8a6a3] rounded-[20px] sm:rounded-[28px] md:rounded-[32px] shadow-[0_20px_60px_rgba(0,0,0,0.3),_inset_0_1px_0_rgba(255,255,255,0.3)] border border-[#8a8885] font-['Space_Grotesk'] w-full max-w-[95vw] sm:max-w-full mx-auto">
			...
			{/* SEARCH PANEL */}
			<div className="w-full max-3xl bg--color-te-orange p-3 sm:p-4 md:p-6 rounded-[16px] sm:rounded-[20px] md:rounded-[24px] shadow-[0_10px_30px_rgba(0,0,0,0.1),_inset_0_1px_0_white/40] border border-[#8a8885]">
				<input
					type="text"
			...
			{/* ALBUM GRID */}
			<div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4 w-full max-w-4xl">
				{displayedAlbums.length > 0 ? (
					displayedAlbums.map((album) => (
						<motion.button
							...
							className="group relative flex flex-col items-center p-2 sm:p-3 md:p-4 bg-[#d4d2cf] rounded-[16px] sm:rounded-[20px] md:rounded-[24px] shadow-[0_2px_0_#8a8889,_0_8px_12px_rgba(0,0,0,0.2),_inset_0_1px_0_rgba(255,255,255,0.5)] active:shadow-[0_2px_0_#8a8885,_0_4px_8px_rgba(0,0,0,0.2),_inset_0_1px_0_rgba(255,255,255,0.5)] active:translate-y-[2px] transition-all border border-white/20">
							<div className="w-full aspect-square mb-2 sm:mb-3 md:mb-4 overflow-hidden rounded-[8px] sm:rounded-[10px] md:rounded-[12px] bg-[#2a2826]">
								<img
									src={album.images[0].url}
									alt={album.name}
									className="w-full h-full rounded-[8px] sm:rounded-[10px] md:rounded-[12px] object-cover"
								/>
							</div>
